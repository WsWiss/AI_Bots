...
class ActionLoadUserData(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        try:
            # Получение последней записи из таблицы users
            last_user = user_db.get_last_user()

            if not last_user:
                dispatcher.utter_message(text="Данных пользователей не найдено.")
                return []

            # Вывести полученные данные
            user_id = last_user.get("user_id")
            tasks = json.loads(last_user.get("tasks", "[]"))

            dispatcher.utter_message(text=f"Последний пользователь: {user_id}\nЗадачи: {tasks}")

            return [SlotSet("user_data", last_user)]
        except Exception as e:
            dispatcher.utter_message(text=f"Ошибка при загрузке данных пользователя: {str(e)}")
        finally:
            user_db.close()
...
class ActionAddTask(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        try:
            if not task:
                dispatcher.utter_message(text="Не удалось распознать задачу.")
                return []
        ...
        try:
            ...
            if task_id == "все":
                ...
                if 0 <= index < len(tasks):
                    ...
                    dispatcher.utter_message(text="Неверный номер задачи.")
                    return []
