...
╚══════════════════════════════════╝{COLORS['end']}""",
  ...
  • {COLORS['underline']}Решить примеры{COLORS['end']}{COLORS['blue']} типа 2+2 или 5*10{COLORS['end']}"""
    ...
    r"расскажи интересный факт": [
        f"""{COLORS['blue']}Выберите категорию:
  {COLORS['green']}• Спорт{COLORS['end']}
  {COLORS['yellow']}• История{COLORS['end']}
  {COLORS['red']}• Космос{COLORS['end']}""",
...
FACTS = {
    "спорт": [
        f"{COLORS['green']}🏸 Бадминтон — самый быстрый ракеточный вид спорта: скорость волана может достигать 270 км/ч.{COLORS['end']}",
        f"{COLORS['green']}🏌️ В стандартном мячике для гольфа 336 выемок.{COLORS['end']}",
        f"{COLORS['green']}🏎️ В Формуле-1 нет болида под номером 13 — после 12 идёт сразу 14.{COLORS['end']}"
    ],
    "история": [
        f"{COLORS['yellow']}🏰 Великая Китайская стена не видна с Луны невооружённым глазом.{COLORS['end']}",
        f"{COLORS['yellow']}📸 Первая фотография была сделана в 1826 году.{COLORS['end']}",
        f"{COLORS['yellow']}⚔️ Арабские цифры были изобретены в Индии.{COLORS['end']}"
    ],
    "космос": [
        f"{COLORS['blue']}🪐 Температура на Венере (465°C) выше, чем на Меркурии.{COLORS['end']}",
        f"{COLORS['blue']}🌌 В космосе нельзя заплакать — слёзы не падают в невесомости.{COLORS['end']}",
        f"{COLORS['blue']}🚀 Сатурн мог бы плавать в воде из-за низкой плотности.{COLORS['end']}"
    ]
}
...
def search_internet(query: str) -> str:
    ...
    try:
        ...
        webbrowser.open(url)
        return f"{COLORS['yellow']}🌍 Выполнен поиск в Google: '{query}'. Проверьте ваш браузер!{COLORS['end']}"
...
async def process_user_input(user_input: str, current_category: str = None, db: Database = None) -> tuple:
    ...
    # Обработка категорий фактов
    if current_category:
        if user_input in FACTS:
            return None, random.choice(FACTS[user_input]), False
        return None, f"{COLORS['red']}🚫 Неизвестная категория. Попробуйте: спорт, история или космос.{COLORS['end']}", False
    ...
    # Факты
    if "факт" in user_input:
        return "fact_category", random.choice(RESPONSES[r"расскажи интересный факт"]), True
